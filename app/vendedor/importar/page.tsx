...
import { IconCheck, IconAlertTriangle, IconCamera } from '@/lib/icons'
        ...
        {categorias && categorias.map((resumen, idx) => (
          <div className="card mb-4" key={idx}>
            <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
              <IconCheck className="w-5 h-5 text-green-600" /> Categoría: {resumen.categoria}
            </h2>
            ...
            {resumen.faltaImagen.length > 0 && (
              <div className="bg-yellow-50 border border-yellow-300 rounded p-3 mb-3">
                <p className="font-bold text-sm mb-2 flex items-center gap-2">
                  <IconAlertTriangle className="w-4 h-4 text-yellow-600" /> Productos sin foto — subila acá directamente:
                </p>
                <ul className="text-sm space-y-2">
                  {resumen.faltaImagen.map(p => (
                    <li key={p.id} className="flex items-center justify-between gap-2 bg-white rounded border border-yellow-200 px-3 py-2">
                      ...
                      {fotosOk.has(p.id) ? (
                        <span className="text-green-700 font-bold text-xs whitespace-nowrap inline-flex items-center gap-1">
                          <IconCheck className="w-3.5 h-3.5" /> Subida
                        </span>
                      ) : subiendoFoto === p.id ? (
                        ...
                        <label className="btn-secondary text-xs cursor-pointer whitespace-nowrap inline-flex items-center gap-1.5">
                          <IconCamera className="w-3.5 h-3.5" /> Subir foto
                          <input
                            type="file"
                            accept="image/*"
                            className="hidden"
